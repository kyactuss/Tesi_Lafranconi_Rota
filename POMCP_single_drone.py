...
class POMCPSolver:
    ...
    def search(self, current_belief_map, drone_position):
...
def draw_elements(screen, belief_map, drone_pos, params, font_sidebar, GRID_WIDTH, CELL_SIZE, steps_taken, pomcp_stats):
    ...
    # Drone (cerchio) - posizione logica (riga, colonna)
    center = (drone_pos[1] * CELL_SIZE + CELL_SIZE // 2, drone_pos[0] * CELL_SIZE + CELL_SIZE // 2)
    pygame.draw.circle(screen, RED, center, CELL_SIZE // 3, 4)
    ...
    while running:
        ...
        if auto_mode:
            ...
            if current_time - auto_timer > AUTO_INTERVAL:
                # POMCP search
                start_time = time.time()
                best_action = solver.search(belief_map.copy(), drone_pos)
                ...
                # Esegui movimento
                drone_pos = execute_move(drone_pos, best_action)

                # Osservazione reale
                obs = get_real_observation(drone_pos, target_pos, params['real_alpha'], params['real_beta'])
        ...
        draw_elements(
            screen, belief_map, drone_pos, params,
