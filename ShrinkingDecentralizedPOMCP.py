...
def get_user_parameters():
    ...
    params_dict = {
        ...
        'execution_horizon': DEFAULT_CONFIG['execution_horizon'],
...
class POMCPSolver:
    def __init__(self, max_time, depth_limit, discount_factor, exploration_const, sensor_alpha, sensor_beta, reward_alpha, explorative_reward, map_size, obstacle_map, drone_id, dist_BFS, r_target, explored_cells, planning_horizon, execution_horizon):
        ...
        self.remaining_threshold = planning_horizon - execution_horizon
    ...
    def search(self, current_belief_map, drone_position, partner_positions=None, partner_plans=None):
        ...
        # Extraction of top branches to generate the POMCP decision tree
        tree_data = self._extract_top_n_tree(root, 0, self.planning_horizon)
        ...
        return best_action, tree_data

    def _extract_top_n_tree(self, node, depth, max_depth):
        if depth >= max_depth or not node.children:
            return None
        
        tree_data = {}
        # Sort actions by Q-value descending
        sorted_actions = sorted(node.q_value_actions.items(), key=lambda x: x[1], reverse=True)
        
        for action, q_val in sorted_actions:
            # Find all child keys (action, obs) corresponding to this action
            matching_keys = [k for k in node.children.keys() if k[0] == action]
            if matching_keys:
                # Find most visited observation branch for this action
                best_key = max(matching_keys, key=lambda k: node.children[k].total_node_visits)
                child_node = node.children[best_key]
                child_data = self._extract_top_n_tree(child_node, depth + 1, max_depth)
                tree_data[action] = {'q_value': q_val, 'child': child_data}
            else:
                tree_data[action] = {'q_value': q_val, 'child': None}
    ...
    def build_virtual_belief_map(self, current_belief_map, my_pos, partner_positions, partner_plans):
        ...
        for partner_id, partner_pos in partner_positions.items():
            ...
            if not (dist_manhattan <= self.remaining_threshold and partner_id > self.drone_id):
...
def worker_pomcp_task(params, belief_map, my_pos, partner_positions, partner_plans, drone_id, obstacle_map, explored_cells):
    ...
    best_action, tree_data = solver.search(virtual_belief_map, my_pos, partner_positions, partner_plans)
...
class DroneAgent:
    ...
    def __init__(self, drone_id, start_pos, params, partner_positions=None):
        ...
        self.active_plan = []           # Buffer of actions to execute
        self.pomcp_tree_data = None     # Store POMCP tree returned by the solver
    ...
    def _compute_spatial_trajectory(self, start_pos, actions):
        trajectory = []
        curr_pos = start_pos
    ...
    def negotiate_best_plan(self, locked_plans_spatial):
        
        horizon = self.params.get('planning_horizon', 7)
        
        def dfs_search(tree_node, current_pos, depth):
            if depth >= horizon or not tree_node:
                return []
            
            # tree_node is a dict of actions: {'N': {'q_value': 1.2, 'child': {...}}, ...}
            # they are already sorted by _extract_top_n_tree, but we can iterate normally
            sorted_actions = sorted(tree_node.items(), key=lambda item: item[1]['q_value'], reverse=True)
            
            for action, data in sorted_actions:
                d = MOVES_DELTA.get(action, (0, 0))
                next_pos = (current_pos[0] + d[0], current_pos[1] + d[1])
                
                # Check collision with locked_plans_spatial AND unplanned partners
                collision = False
                for p_id, p_pos in self.partner_positions.items():
                    if p_id in locked_plans_spatial:
                        p_traj = locked_plans_spatial[p_id]
                        if depth < len(p_traj):
                            if p_traj[depth] == next_pos: # Vertex collision
                                collision = True
                                break
                            # Trailing (e implicitamente swap) da parte mia rispetto a lui
                            p_prev_pos = p_traj[depth-1] if depth > 0 else p_pos
                            if p_prev_pos == next_pos:
                                collision = True
                                break
                            # Trailing da parte sua rispetto a me (lui viene dove sono io ora)
                            if p_traj[depth] == current_pos:
                                collision = True
                                break
                    else:
                        # I droni con ID maggiore non hanno ancora un piano.
                        # Trattiamo la loro posizione di partenza come inviolabile per tutto l'orizzonte.
                        if p_pos == next_pos:
                            collision = True
                            break
                if collision:
                ...
                # Safe, recurse
                sub_plan = dfs_search(data['child'], next_pos, depth + 1)
                if sub_plan is not None:
                    return [action] + sub_plan
            
            return None # Dead end

        plan = dfs_search(self.pomcp_tree_data, self.pos, 0)
        if not plan:
            plan = ['Stay'] * horizon
            
        self.active_plan = plan
        return self._compute_spatial_trajectory(self.pos, self.active_plan)
    ...
    def send_observation_and_future_plan(self):
        ...
        future_plan = self.active_plan.copy() if hasattr(self, 'active_plan') else []       # Exclude first action (already executed) from communicated plan
    ...
    def update_belief_from_all_obs(self):
        ...
        for pos, obs in all_observations:
            if isinstance(obs, dict) and 'type' in obs and 'pos' in obs:    # Check if observation is a trace
                ...
                # Global plan clear on positive target observation during POMCP mode
    ...
    try:
        while running:
            ...
            if auto_mode and (time.monotonic() - last_step_time) >= move_interval_sec:
                ...
                # Check if any drone in POMCP mode has an active plan with remaining length <= threshold, if so we need to replan for all POMCP drones
                exec_horizon = params['execution_horizon']
                plan_horizon = params['planning_horizon']
                remaining_threshold = plan_horizon - exec_horizon

                needs_planning = False
                for drone in drones:
                    if getattr(drone, 'search_mode', None) == 'POMCP':
                        if len(getattr(drone, 'active_plan', [])) <= remaining_threshold:
                            needs_planning = True
                            break

                tasks = []
                for drone in drones:
                    if getattr(drone, 'search_mode', None) == 'POMCP' and needs_planning:
                ...
                if tasks:
                    ...
                    for drone in drones:
                        if getattr(drone, 'search_mode', None) == 'POMCP':
                            drone.planned_result = results[res_idx]
                            drone.pomcp_tree_data = results[res_idx]['tree_data']
                            res_idx += 1

                    # Cascade Negotiation
                    locked_plans_spatial = {}
                    sorted_drones = sorted(drones, key=lambda d: d.id)      # Sort drones by ID
                    for drone in sorted_drones:
                        if getattr(drone, 'search_mode', None) == 'POMCP':
                            spatial_plan = drone.negotiate_best_plan(locked_plans_spatial)
                            locked_plans_spatial[drone.id] = spatial_plan

                # Action extraction & TSP execution for all drones
                for drone in drones:
                    if getattr(drone, 'search_mode', None) == 'POMCP':
                        drone.final_action = drone.active_plan.pop(0) if getattr(drone, 'active_plan', []) else 'Stay'
                        
                    elif getattr(drone, 'search_mode', None) == 'TSP':
                ...
                # Reset timer AFTER computing and updating everything. 
                # This guarantees visually seeing the frame for 'move_interval_sec' seconds.
