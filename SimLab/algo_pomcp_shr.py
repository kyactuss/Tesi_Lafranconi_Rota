...
class POMCPSolver:
    ...
    def search(self, current_belief_map, drone_position, partner_positions=None, partner_plans=None):
        ...
        return best_action, tree_data, metrics
...
def worker_pomcp_task(params, belief_map, my_pos, partner_positions, partner_plans, drone_id, obstacle_map, explored_cells):
    ...
    solver = POMCPSolver(
        ...
        planning_horizon=params.get('planning_horizon', 7),
        execution_horizon=params.get('execution_horizon', 3)
    )
    
    virtual_belief_map = solver.build_virtual_belief_map(belief_map, my_pos, partner_positions, partner_plans)
    best_action, tree_data, metrics = solver.search(virtual_belief_map, my_pos, partner_positions, partner_plans)
...
class DroneAgent:
    ...
    def send_observation_and_future_plan(self):
        future_plan = self.active_plan.copy() if hasattr(self, 'active_plan') else []       
        return {
            'id': self.id,
            'pos': self.pos,
...
def init_graphics(params):
    ...
    pygame.display.set_caption("Multi-Drone POMCP_SHR Decentralized")
...
def run_simulation(params):
    ...
    try:
        while running:
            ...
            # --- INIZIO MODIFICA 1: Ignora il timer se use_gui è False ---
            ...
            # --- FINE MODIFICA 1 ---
                ...
                exec_horizon = params.get('execution_horizon', 3)
                plan_horizon = params.get('planning_horizon', 7)
                ...
                if tasks:
                    ...
                    for drone in drones:
                        if getattr(drone, 'search_mode', None) == 'POMCP' and needs_planning:
                            ...
                            # SALVATAGGIO METRICHE SHR-POMCP SOLO QUANDO SCATTA IL MONTECARLO
                            metric = results[res_idx]['metrics']
                ...
                for drone in drones:
                    ...
                    had_conflict = (drone.final_action != result.get('best_action', 'Stay')) if result else False
                    ui_stats['drones'][drone.id].update({
                        'obs': drone.observation,
                        'depth': result.get('depth', 0) if result else 0,
                        'visits': result.get('visits', 0) if result else 0,
                        'nodes': result.get('nodes_created', 0) if result else 0,
                        'best': result.get('best_action', '-') if result else '-',
                ...
                if drones[0].belief_map.max() >= 0.95:
                    ...
                    # --- INIZIO MODIFICA 2: Chiudi pygame solo se è aperto ---
                    if use_gui:
                        pygame.quit()
                    # --- FINE MODIFICA 2 ---
                    return step_counter, None, all_drones_metrics
                
                # --- INIZIO MODIFICA 3: last_step_time è stato rimosso da qui e gestito al passo 1 ---
                
            # --- INIZIO MODIFICA 4: Rendering e gestione eventi spostati qui e subordinati a use_gui ---
            if use_gui:
