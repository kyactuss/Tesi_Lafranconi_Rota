...
class DroneAgent:
    def __init__(self, drone_id, start_pos, params, partner_positions=None):
        self.id = drone_id      
        self.params = params    
        self.search_mode = 'TSP' if params['map_type'] == 1 else 'AUCTION'    
...
def run_simulation(params):
    ...
    running = True
    auto_mode = True            # Inizio in automatico forzato
    step_counter = 0
    ...
    while running:
            ...
            # --- FINE MODIFICA 1 ---
                ...
                # --- INIZIO MODIFICA 2: Il render_frame è stato rimosso da qui e spostato sotto ---

                # Simulation termination condition
                if drones[0].belief_map.max() >= 0.95:
                    print(f"\n TARGET TROVATO in {step_counter} step! (probabilità > 95%)")
                    # --- INIZIO MODIFICA 3: Chiudi pygame solo se è aperto ---
                    if use_gui:
            ...
            if use_gui:
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT: 
                        pygame.quit()
                        return -1, []  # Interrotto dall'utente
